...
- **Lección 4: Verificación de Syntax (SyntaxError)**:
    - **Problema**: Una redeclaración accidental (`const lang`) en el mismo bloque de `engine.js` puede tumbar todo el plugin.
    - **Lección**: Siempre verificar que las variables de normalización no se declaren dos veces tras una refactorización masiva de bloques.

---

### [2026-04-10] Arquitectura de Estabilidad Global y el "Misterio de los IDs" (v1.6.30)

- **El Caso Miénteme (Lección Final)**: Lo que parecía un error de IDs corruptos resultó ser una falta de robustez en el scraper de Embed69.
    - **Causa Real**: El scraper anterior era demasiado estricto con las etiquetas (`LAT`, `ESP`) y las cabeceras. Si el sitio movía el contenido de sección, el plugin fallaba.
    - **Solución Maestra**: Se implementó una **Captura Amplia**. El scraper ahora captura todas las secciones de video sin excepción y delega la normalización y filtrado al Motor Central (`engine.js`). 
    - **Resultado**: Al ser más tolerante, el plugin encontró la serie sin necesidad de parches manuales de IDs.

- **Lección 1: Scrapers "Ciegos" pero Robustos**: 
    - Un scraper no debe intentar ser inteligente con los idiomas. Su único trabajo es recolectar todos los enlaces posibles.
    - La inteligencia (Filtrado Latino, Normalización) debe vivir en el núcleo central para asegurar consistencia en los 15 proveedores.

- **Lección 2: El Resolutor Universal como Escudo**:
    - Centralizar los resolutores en `src/utils/resolvers.js` ha demostrado ser la mejor decisión del proyecto. Arreglar un servidor ahí es como vacunar a todo el plugin contra caídas de sitios de streaming.

- **Lección 3: El Mapeador de IDs como Red de Seguridad**:
    - Aunque Miénteme no lo necesitó finalmente, `src/utils/id_mapper.js` se queda como una herramienta de "emergencia". Si una serie futura tiene desfases (como *Scrubs*), tenemos un punto único de intervención rápida que no requiere editar la lógica del scraper.

- **Estado del Proyecto**: El plugin alcanza su madurez arquitectónica en la v1.6.30. La estructura es ahora modular, resiliente y fácil de escalar a nuevos proveedores.
