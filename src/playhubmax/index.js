const { fetchJson } = require('../utils/http.js');
...
async function getContentDetail(uuid) {
    try {
        const data = await fetchJson(`${PHM_API}/en/contents/${uuid}`, { headers: API_HEADERS });
        return data;
    } catch { return {}; }
}
...
async function getStreams(tmdbId, mediaType, season, episode, title) {
    try {
        ...
        const streams = (await Promise.allSettled(finalSources.map(async (s) => {
            ...
            return {
                ...
                url: finalUrl,
                quality: '1080p',
                headers: (result && result.headers) ? result.headers : { 'User-Agent': UA, 'Referer': 'https://www.playhubmax.com/' }
            };
        })))
        .filter(r => r.status === 'fulfilled' && r.value)
        .map(r => r.value);

        return await finalizeStreams(streams, 'PlayHubMax', mediaTitle);
