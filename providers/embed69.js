 ...
 * Generated: 2026-04-13T21:55:02.202Z
 */
  ...
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (allDataLinks.length === 0)
        return [];
      const byLang = {};
      allDataLinks.forEach((s2) => {
        if (s2.video_language) {
          const langKey = s2.video_language.toUpperCase();
          if (!byLang[langKey])
            byLang[langKey] = s2;
          else {
            const existing = byLang[langKey].sortedEmbeds || byLang[langKey].embeds || [];
            const newOnes = s2.sortedEmbeds || s2.embeds || [];
          ...
          if (payload && payload.link) {
            ...
            const resolutionPromise = Promise.race([
              resolveEmbed(url).then((res) => {
                return __spreadProps(__spreadValues({}, res || {}), { langLabel, serverLabel: sLabel, headers: __spreadProps(__spreadValues({}, res && res.headers ? res.headers : {}), { "User-Agent": UA4 }) });
              }),
