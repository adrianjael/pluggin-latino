 ...
 * Generated: 2026-04-14T15:22:48.735Z
 */
...
function getStreams(tmdbId, mediaType, season, episode, title, year) {
  return __async(this, null, function* () {
    try {
      console.log(`[Embed69] Iniciando b\xFAsqueda para: ${title} (${year})`);
      const imdbId = yield getCorrectImdbId(tmdbId, mediaType, title, year);
      if (!imdbId) {
      ...
      if (!response.ok) {
        console.log(`[Embed69] Error en respuesta: ${response.status}`);
        return [];
      }
