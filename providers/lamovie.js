 ...
 * Generated: 2026-04-12T23:55:02.125Z
 */
...
function formatPipedUrl(url, referer) {
  ...
  try {
    ...
    return `${url}|Referer=${referer}|Origin=${origin}|User-Agent=${UA3}`;
  } catch (e) {
    return `${url}|Referer=${referer}|User-Agent=${UA3}`;
  }
}
  ...
  return __async(this, null, function* () {
    ...
    try {
      const { data: html } = yield axios3.get(url, {
        timeout: 8e3,
        headers: { "User-Agent": UA5, "Accept": "text/html" },
...
function getEpisodeId(seriesId, seasonNum, episodeNum) {
  return __async(this, null, function* () {
    ...
    try {
      const { data } = yield axios3.get(url, { timeout: 12e3, headers: HEADERS });
...
function getStreams(tmdbId, mediaType, season, episode, title) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (!mediaTitle)
        return [];
      const found = yield findBySlug({ title: mediaTitle }, mediaType);
      if (!found)
        return [];
      ...
      const { data } = yield axios3.get(`${BASE_URL}/wp-api/v1/player?postId=${targetId}&demo=0`, { timeout: 6e3, headers: HEADERS });
      ...
      const streams = (yield Promise.allSettled(data.data.embeds.map((embed) => __async(this, null, function* () {
        const result = yield resolveEmbed2(embed.url);
        if (!result || !result.url)
          return null;
        return {
          langLabel: embed.lang || "Latino",
          serverLabel: getServerName(embed.url),
          url: result.url,
          quality: result.quality || "1080p",
          headers: result.headers || {}
        };
      })))).filter((r) => r.status === "fulfilled" && r.value).map((r) => r.value);
      return yield finalizeStreams(streams, "LaMovie", mediaTitle);
    } catch (e) {
