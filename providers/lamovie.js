 ...
 * Generated: 2026-04-13T02:13:49.498Z
 */
...
function getIdBySlug(category, slug) {
  return __async(this, null, function* () {
    ...
    try {
      const { data: html } = yield axios2.get(url, {
        timeout: 8e3,
...
function getEpisodeId(seriesId, seasonNum, episodeNum) {
  return __async(this, null, function* () {
    ...
    try {
      const { data } = yield axios2.get(url, { timeout: 12e3, headers: HEADERS });
...
function getStreams(tmdbId, mediaType, season, episode, title) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      const { data } = yield axios2.get(`${BASE_URL}/wp-api/v1/player?postId=${targetId}&demo=0`, { timeout: 6e3, headers: HEADERS });
      ...
      const streams = (yield Promise.allSettled(data.data.embeds.map((embed) => __async(this, null, function* () {
        const result = yield resolveEmbed(embed.url);
        if (!result || !result.url)
          return null;
        return {
