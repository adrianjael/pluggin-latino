 ...
 * Generated: 2026-04-13T05:03:42.828Z
 */
    ...
    function resolve9(url) {
      return __async(this, null, function* () {
        try {
          ...
          let response = yield axios7.get(url, { headers: defaultHeaders, timeout: 8e3 });
          let html = response.data;
          if (typeof html === "string" && (html.includes("Redirecting...") || html.length < 1500)) {
            ...
            if (redirectMatch) {
              ...
              response = yield axios7.get(newUrl, { headers: __spreadProps(__spreadValues({}, defaultHeaders), { "Referer": url }), timeout: 8e3 });
              html = response.data;
            }
          }
...
function getStreams(tmdbId, mediaType, season, episode, title) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      const { data: searchJson } = yield axios6.get(SEARCH_BASE + encodeURIComponent(searchTitle), { timeout: 8e3 });
      ...
      for (const entry of entries) {
        ...
        if (!normEntry.includes(normTitle))
          continue;
        const { data: html } = yield axios6.get(entryUrl, { timeout: 8e3 });
