 ...
 * Generated: 2026-04-10T23:20:26.462Z
 */
...
function getDirectCdnHeaders(url) {
  ...
  const UA11 = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36";
  if (s.includes("acek-cdn.com") || s.includes("minochinos.com") || s.includes("masukestin.com")) {
    return { "User-Agent": UA11, "Referer": "https://vidhide.com/", "Origin": "https://vidhide.com" };
  }
  if (s.includes("r66nv9ed.com") || s.includes("filemoon")) {
    return { "User-Agent": UA11, "Referer": "https://arbitrarydecisions.com/", "Origin": "https://arbitrarydecisions.com" };
  }
  if (s.includes("cloudwindow-route.com") || s.includes("awish.pro") || s.includes("streamwish")) {
    return { "User-Agent": UA11, "Referer": "https://streamwish.to/", "Origin": "https://streamwish.to" };
  }
  return null;
}
...
function registerClick(token, cookie, playerUrl) {
  return __async(this, null, function* () {
    try {
      ...
      yield import_axios10.default.post(`${BASE_URL}/s.php`, body, {
      ...
      console.log("[SoloLatino] Click registrado correctamente.");
    } catch (e) {
      console.log("[SoloLatino] Error al registrar click:", e.message);
    }
  });
}
function getDirectStream(serverId, token, cookie, playerUrl) {
  return __async(this, null, function* () {
    try {
      ...
      const { data } = yield import_axios10.default.post(`${BASE_URL}/s.php`, body, {
        headers: __spreadProps(__spreadValues({}, HEADERS), {
          "cookie": cookie,
          "origin": BASE_URL,
          "referer": playerUrl,
...
function getSessionData(slug) {
  return __async(this, null, function* () {
    try {
      const url = `${BASE_URL}/f/${slug}`;
      console.log(`[SoloLatino] Iniciando sesi\xF3n: ${url}`);
      const response = yield import_axios10.default.get(url, { headers: HEADERS });
