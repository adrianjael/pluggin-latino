 ...
 * Generated: 2026-04-13T04:37:03.288Z
 */
    ...
    function resolve9(url) {
      return __async(this, null, function* () {
        try {
          ...
          const response = yield axios6.get(url, { headers: { "User-Agent": UA5 }, timeout: 8e3 });
          let html = response.data;
          if (html.indexOf("Redirecting") !== -1 || html.length < 1500) {
            const rm = html.match(/window\.location\.href\s*=\s*['"]([^'"]+)['"]/i);
            if (rm) {
              const res2 = yield axios6.get(rm[1], { headers: { "User-Agent": UA5 }, timeout: 8e3 });
              html = res2.data;
            }
          }
