 ...
 * Generated: 2026-04-12T23:57:44.205Z
 */
...
function getImdbId(tmdbId, mediaType) {
  return __async(this, null, function* () {
    try {
      ...
      const { data } = yield axios3.get(url, { timeout: 5e3, headers: { "User-Agent": UA5 } });
...
function getEmbeds(slug, mediaType, season, episode) {
  return __async(this, null, function* () {
    try {
      ...
      const { data: html } = yield axios3.get(`${BASE_URL}${path}`, { timeout: 8e3, headers: HTML_HEADERS });
      ...
      for (const slug of slugVariants) {
        ...
        for (const lang of [0, 1, 2]) {
          ...
          for (const url of urls) {
            try {
              const result = yield resolveEmbed2(url);
              if (result && result.url) {
                allStreams.push({
