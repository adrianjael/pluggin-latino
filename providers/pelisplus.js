...
function normalizeTitle(t) {
  if (!t)
    return "";
  return t.toLowerCase().replace(/[├í├á├ñ├ó]/g, "a").replace(/[├®├¿├½├¬]/g, "e").replace(/[├¡├¼├»├«]/g, "i").replace(/[├│├▓├Â├┤]/g, "o").replace(/[├║├╣├╝├╗]/g, "u").replace(/├▒/g, "n").replace(/[^a-z0-9\s]/g, " ").replace(/\s+/g, " ").trim();
}
  ...
  return __async(this, null, function* () {
    try {
      ...
      if (rawResults.length === 0) {
        console.log("[PelisPlusHD] Probando fallback span lid (series)...");
        const spanRegex = /<span[^>]*lid="(\d+)"[^>]*url="([^"]+)"/gi;
        let sM;
        while ((sM = spanRegex.exec(pageHtml)) !== null) {
          const id = sM[1];
          const serverUrl = sM[2];
          const liNameRegex = new RegExp(`<li[^>]*data-id="${id}"[^>]*>[\\s\\S]*?<a[^>]*>(.*?)<\/a>`, "i");
          const liNameMatch = pageHtml.match(liNameRegex);
          const serverName = liNameMatch ? liNameMatch[1].trim() : `Server${id}`;
          rawResults.push({ serverUrl, serverName, language: "Espa\xF1ol Latino" });
        }
        if (rawResults.length > 0) {
          console.log(`[PelisPlusHD] Span lid: ${rawResults.length} fuentes encontradas.`);
        }
      }
