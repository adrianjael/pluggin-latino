 ...
 * Generated: 2026-04-13T21:55:02.234Z
 */
...
function getOnlineStreams(rawHtml) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (videoLinksMatch) {
        ...
        for (const obj of objects) {
          ...
          if (["hqq", "netu", "prueba", "embedseek", "desu"].some((s2) => lowerServer.includes(s2) || lowerUrl.includes(s2)))
            continue;
    ...
    return results.filter((s2) => s2 !== null);
  });
}
  ...
  return __async(this, null, function* () {
    try {
      ...
      if (upperHtml.includes(" CALIDAD CAM") || upperHtml.includes(">CAM<"))
        ...
        siteQuality = "TS";
      const finalStreams = onlineStreams.map((s2) => __spreadProps(__spreadValues({}, s2), { siteQuality }));
