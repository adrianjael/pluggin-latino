 ...
 * Generated: 2026-04-12T23:57:44.153Z
 */
      ...
      if (videoLinksMatch) {
        ...
        for (const obj of objects) {
          ...
          if (lowerUrl.includes("hqq.tv") || lowerUrl.includes("waaw.to") || lowerUrl.includes("embedseek.com") || lowerUrl.includes("desu"))
            continue;
          const result = yield resolveEmbed2(cleanUrl);
          if (result && result.url) {
            streams.push({
              name: "PelisGo",
              langLabel: "Latino",
              ...
              headers: result.headers || { "User-Agent": UA5, "Referer": BASE }
            });
            ...
            streams.push({
              ...
              url: cleanUrl,
              quality: "1080p",
              headers: { "User-Agent": UA5, "Referer": BASE }
            });
          }
        }
      }
    ...
    try {
      ...
      while ((m = globalRegex.exec(rawHtml)) !== null) {
        ...
        if (directUrl && !seenUrls.has(directUrl)) {
          ...
          const resolved = yield resolveEmbed2(directUrl);
          if (resolved && resolved.url) {
            streams.push({
              ...
              headers: resolved.headers || { "User-Agent": UA5, "Referer": directUrl }
            });
          }
        }
      }
